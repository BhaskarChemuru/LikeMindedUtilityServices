...
<!-- Main Section Begins --><!-- This will clear any floats from previous Section -->
  <div class="Main">
    <div>
      <h1 class="bold left btPad10 tpPad6">Billing History</h1>
    </div>
  ...
  //<![CDATA[
    jQuery(document).ready(function () {
       /* var mydata = [
        ...
        var mydata =<%=request.getAttribute("billhistoryData")%>;

        $("#list").jqGrid({
            datatype: "local",
            data: mydata,
            colNames:['Bill Due Date','Total Bill Amount'],
            colModel:[
                {name:'duedate',index:'duedate', width:350, align:'center', sorttype:'date', formatter:'date', formatoptions: {newformat:'d-M-Y'}},
                {name:'amountdue',index:'amountdue', width:360, formatter:'number', align:"center"}
            ],
            rowNum:10,
            rowList:[5,10,20],
            pager: '#pager',
            ...
            viewrecords: true,
            sortorder: "desc",
            caption:"Customer Billing History",
            height: "100%",
			width: "875"
